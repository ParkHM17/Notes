...
- 不同点
  ...
  - `@Resource`有两个属性：**`name`**和**`type`**。Spring将`@Resource`的`name`属性解析为Bean的名字，而`type`属性则解析Bean的类型。如果使用`name`属性，则使用**byName的自动注入策略**；使用`type`属性时则使用**byType的自动注入策略**；如果既不指定`name`也不指定`type`，将**通过反射机制使用byName的自动注入策略**。
