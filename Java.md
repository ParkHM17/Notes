...
- 作为成员变量，包装类型不赋值就是`null`，而基本数据类型有默认值。
- 包装类型可用于**泛型**，而基本数据类型不可以。
...
| :----------: | :--------------: | :----------------------------------------------------------: |
| 基本数据类型 |    比较**值**    |                             :x:                              |
| 引用数据类型 | 比较**内存地址** | 没有重写：使用的是父类`Object`的`equals()`方法，等价于`==`<br />重写：比较两个对象中的**属性是否相等** |
...
当把对象加入`HashSet`时，`HashSet`会先通过`hashCode()`计算对象的哈希码来**判断对象加入的位置**，同时也会与其他已经加入的对象的哈希码比较；如果没有相符的哈希码，**会假设对象没有重复出现**；如果发现有相同哈希码的对象，这时会调用`equals()`方法来检查相等的对象**是否真的相同**。如果两者相同，`HashSet`就不会让其加入；如果不同的话，就会重新散列到其他位置。这样就大大减少了`equals()`的次数，相应地也大大提高了执行速度。
...
| :--------: | :----------------------------------: | :-------------: | :---------------------------------------------------: |
|   可变性   |                 :x:                  |       :o:       |                          :o:                          |
| 线程安全性 |                 :o:                  |       :x:       | :o:（对方法加了**同步锁**或者对调用的方法加了同步锁） |
...
`Annotation`（注解）是Java 5开始引入的新特性，可以看作是一种特殊的注释，主要用于**修饰类、方法或者变量**。注解本质是一个继承了`Annotation`的特殊接口：
...
> 参考链接：[掘金](https://juejin.cn/post/6931543528971436046)

|                            |          select          |           poll           |                            epoll                             |
| :------------------------: | :----------------------: | :----------------------: | :----------------------------------------------------------: |
|        底层数据结构        |    数组存储文件描述符    |    链表存储文件描述符    |    红黑树存储监控的文件描述符，双链表存储就绪的文件描述符    |
| 如何从fd数据中获取就绪的fd |        遍历fd_set        |         遍历链表         |                             回调                             |
|         时间复杂度         |           O(n)           |           O(n)           | 当有就绪事件时，系统注册的回调函数就会被调用，将就绪的fd放入到就绪链表中，O(1) |
...
在JDK动态代理机制中，`InvocationHandler`接口和`Proxy`类是核心。`Proxy`类中使用频率最高的方法是：`newProxyInstance()`，这个方法主要用于生成一个代理对象。
...
### 11.1 `Collection`与`Collections`的区别

> 参考链接：[阿里云开发者社区](https://developer.aliyun.com/article/330954)

- `java.util.Collection`是一个**集合接口（集合类的一个顶级接口）**，提供了对集合对象进行基本操作的通用接口方法。`Collection`接口有很多具体的实现，其直接继承接口有`List`与`Set`。
- `java.util.Collections`是一个**包装类（工具类/帮助类）**，它包含有各种有关集合操作的**静态多态方法**。此类**不能实例化**，就像一**个工具类**，用于对集合中元素进行排序、搜索以及线程安全等各种操作，服务于Java的`Collection`框架。

### 11.2 `final`有哪些用法？

> 参考：/Reference/java面经/基础篇/19

- 被`final`修饰的类不可被继承。
- 被`final`修饰的方法不可以被重写，并且JVM会尝试将其内联，提高运行效率。
- 被`final`修饰的变量不可以被改变。**如果修饰引用，表示引用不可变，但引用指向的内容可变**。特别地，被`final`修饰的常量（成员变量）在编译阶段会存入常量池中。

### 11.3 `static`有哪些用法？

> 参考：/Reference/java面经/基础篇/20

- **被`static`修饰的成员变量和方法为静态变量和静态方法**，属于类的静态资源，为类实例所共享。
- 被`static`修饰的代码块为静态代码块，多用于初始化操作。
- `static`也可以用来修饰内部类，称为静态内部类。
- `import static ……`，即静态导包，在JDK 1.5之后引入的新特性，用来指定导入某个类中的静态资源，并且不需要使用类名，可以直接使用资源名。

### 11.4 `a = a + b`与`a += b` 有什么区别？

> 参考：/Reference/java面经/基础篇/22

`+=`操作符会进行隐式自动类型转换，**`a += b`隐式地将加操作的结果类型强制转换为持有结果的类型**，而`a = a + b`不会自动进行类型转换。

```java
byte a = 127;
byte b = 127;
// 以下代码报错，因为右边操作结果为int类型
b = a + b;
// 以下代码不报错，但是结果不对，因为超出bytele
b += a;
```
